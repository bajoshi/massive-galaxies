...
def get_all_redshifts(current_id, current_field, current_ra, current_dec, current_specz,\
    ...
    # Yes this has to be read in every time this function is called.
    # This is because if this function does run successfully (as in the normal
    ...
    zs_arr_fl = np.load(savedir_photoz + 'firstlight_zs_arr.npy')

    zp_minchi2_arr_fl = np.load(savedir_photoz + 'firstlight_zp_minchi2_arr.npy')
    zp_arr_fl = np.load(savedir_photoz + 'firstlight_zp_arr.npy')
    zp_zerr_low_arr_fl = np.load(savedir_photoz + 'firstlight_zp_zerr_low_arr.npy')
    zp_zerr_up_arr_fl = np.load(savedir_photoz + 'firstlight_zp_zerr_up_arr.npy')
    zp_min_chi2_arr_fl = np.load(savedir_photoz + 'firstlight_zp_min_chi2_arr.npy')
    zp_bestalpha_arr_fl = np.load(savedir_photoz + 'firstlight_zp_bestalpha_arr.npy')
    zp_model_idx_arr_fl = np.load(savedir_photoz + 'firstlight_zp_model_idx_arr.npy')
    zp_age_arr_fl = np.load(savedir_photoz + 'firstlight_zp_age_arr.npy')
    zp_tau_arr_fl = np.load(savedir_photoz + 'firstlight_zp_tau_arr.npy')
    zp_av_arr_fl = np.load(savedir_photoz + 'firstlight_zp_av_arr.npy')

    zspz_minchi2_arr_fl = np.load(savedir_spz + 'firstlight_zspz_minchi2_arr.npy')
    zspz_arr_fl = np.load(savedir_spz + 'firstlight_zspz_arr.npy')
    zspz_zerr_low_arr_fl = np.load(savedir_spz + 'firstlight_zspz_zerr_low_arr.npy')
    zspz_zerr_up_arr_fl = np.load(savedir_spz + 'firstlight_zspz_zerr_up_arr.npy')
    zspz_min_chi2_arr_fl = np.load(savedir_spz + 'firstlight_zspz_min_chi2_arr.npy')
    zspz_bestalpha_arr_fl = np.load(savedir_spz + 'firstlight_zspz_bestalpha_arr.npy')
    zspz_model_idx_arr_fl = np.load(savedir_spz + 'firstlight_zspz_model_idx_arr.npy')
    zspz_age_arr_fl = np.load(savedir_spz + 'firstlight_zspz_age_arr.npy')
    zspz_tau_arr_fl = np.load(savedir_spz + 'firstlight_zspz_tau_arr.npy')
    zspz_av_arr_fl = np.load(savedir_spz + 'firstlight_zspz_av_arr.npy')

    zg_minchi2_arr_fl = np.load(savedir_grismz + 'firstlight_zg_minchi2_arr.npy')
    zg_arr_fl = np.load(savedir_grismz + 'firstlight_zg_arr.npy')
    zg_zerr_low_arr_fl = np.load(savedir_grismz + 'firstlight_zg_zerr_low_arr.npy')
    zg_zerr_up_arr_fl = np.load(savedir_grismz + 'firstlight_zg_zerr_up_arr.npy')
    zg_min_chi2_arr_fl = np.load(savedir_grismz + 'firstlight_zg_min_chi2_arr.npy')
    zg_bestalpha_arr_fl = np.load(savedir_grismz + 'firstlight_zg_bestalpha_arr.npy')
    zg_model_idx_arr_fl = np.load(savedir_grismz + 'firstlight_zg_model_idx_arr.npy')
    zg_age_arr_fl = np.load(savedir_grismz + 'firstlight_zg_age_arr.npy')
    zg_tau_arr_fl = np.load(savedir_grismz + 'firstlight_zg_tau_arr.npy')
    zg_av_arr_fl = np.load(savedir_grismz + 'firstlight_zg_av_arr.npy')

    assert len(id_arr_fl) == len(field_arr_fl)
    assert len(id_arr_fl) == len(zs_arr_fl)

    assert len(id_arr_fl) == len(zp_minchi2_arr_fl)
    assert len(id_arr_fl) == len(zp_arr_fl)
    assert len(id_arr_fl) == len(zp_zerr_low_arr_fl)
    assert len(id_arr_fl) == len(zp_zerr_up_arr_fl)
    assert len(id_arr_fl) == len(zp_min_chi2_arr_fl)
    assert len(id_arr_fl) == len(zp_bestalpha_arr_fl)
    assert len(id_arr_fl) == len(zp_model_idx_arr_fl)
    assert len(id_arr_fl) == len(zp_age_arr_fl)
    assert len(id_arr_fl) == len(zp_tau_arr_fl)
    assert len(id_arr_fl) == len(zp_av_arr_fl)

    assert len(id_arr_fl) == len(zspz_minchi2_arr_fl)
    assert len(id_arr_fl) == len(zspz_arr_fl)
    assert len(id_arr_fl) == len(zspz_zerr_low_arr_fl)
    assert len(id_arr_fl) == len(zspz_zerr_up_arr_fl)
    assert len(id_arr_fl) == len(zspz_min_chi2_arr_fl)
    assert len(id_arr_fl) == len(zspz_bestalpha_arr_fl)
    assert len(id_arr_fl) == len(zspz_model_idx_arr_fl)
    assert len(id_arr_fl) == len(zspz_age_arr_fl)
    assert len(id_arr_fl) == len(zspz_tau_arr_fl)
    assert len(id_arr_fl) == len(zspz_av_arr_fl)

    assert len(id_arr_fl) == len(zg_minchi2_arr_fl)
    assert len(id_arr_fl) == len(zg_arr_fl)
    assert len(id_arr_fl) == len(zg_zerr_low_arr_fl)
    assert len(id_arr_fl) == len(zg_zerr_up_arr_fl)
    assert len(id_arr_fl) == len(zg_min_chi2_arr_fl)
    assert len(id_arr_fl) == len(zg_bestalpha_arr_fl)
    assert len(id_arr_fl) == len(zg_model_idx_arr_fl)
    assert len(id_arr_fl) == len(zg_age_arr_fl)
    assert len(id_arr_fl) == len(zg_tau_arr_fl)
    assert len(id_arr_fl) == len(zg_av_arr_fl)
    ...
    id_arr_fl = np.append(id_arr_fl, current_id)
    field_arr_fl = np.append(field_arr_fl, current_field)
    zs_arr_fl = np.append(zs_arr_fl, current_specz)
    
    zp_minchi2_arr_fl = np.append(zp_minchi2_arr_fl, zp_minchi2)
    zp_arr_fl = np.append(zp_arr_fl, zp)
    zp_zerr_low_arr_fl = np.append(zp_zerr_low_arr_fl, zp_zerr_low)
    zp_zerr_up_arr_fl = np.append(zp_zerr_up_arr_fl, zp_zerr_up)
    zp_min_chi2_arr_fl = np.append(zp_min_chi2_arr_fl, zp_min_chi2)
    zp_bestalpha_arr_fl = np.append(zp_bestalpha_arr_fl, zp_bestalpha)
    zp_model_idx_arr_fl = np.append(zp_model_idx_arr_fl, zp_model_idx)
    zp_age_arr_fl = np.append(zp_age_arr_fl, zp_age)
    zp_tau_arr_fl = np.append(zp_tau_arr_fl, zp_tau)
    zp_av_arr_fl = np.append(zp_av_arr_fl, zp_av)
    
    zspz_minchi2_arr_fl = np.append(zspz_minchi2_arr_fl, zspz_minchi2)
    zspz_arr_fl = np.append(zspz_arr_fl, zspz)
    zspz_zerr_low_arr_fl = np.append(zspz_zerr_low_arr_fl, zspz_zerr_low)
    zspz_zerr_up_arr_fl = np.append(zspz_zerr_up_arr_fl, zspz_zerr_up)
    zspz_min_chi2_arr_fl = np.append(zspz_min_chi2_arr_fl, zspz_min_chi2)
    zspz_bestalpha_arr_fl = np.append(zspz_bestalpha_arr_fl, zspz_bestalpha)
    zspz_model_idx_arr_fl = np.append(zspz_model_idx_arr_fl, zspz_model_idx)
    zspz_age_arr_fl = np.append(zspz_age_arr_fl, zspz_age)
    zspz_tau_arr_fl = np.append(zspz_tau_arr_fl, zspz_tau)
    zspz_av_arr_fl = np.append(zspz_av_arr_fl, zspz_av)
    
    zg_minchi2_arr_fl = np.append(zg_minchi2_arr_fl, zg_minchi2)
    zg_arr_fl = np.append(zg_arr_fl, zg)
    zg_zerr_low_arr_fl = np.append(zg_zerr_low_arr_fl, zg_zerr_low)
    zg_zerr_up_arr_fl = np.append(zg_zerr_up_arr_fl, zg_zerr_up)
    zg_min_chi2_arr_fl = np.append(zg_min_chi2_arr_fl, zg_min_chi2)
    zg_bestalpha_arr_fl = np.append(zg_bestalpha_arr_fl, zg_bestalpha)
    zg_model_idx_arr_fl = np.append(zg_model_idx_arr_fl, zg_model_idx)
    zg_age_arr_fl = np.append(zg_age_arr_fl, zg_age)
    zg_tau_arr_fl = np.append(zg_tau_arr_fl, zg_tau)
    zg_av_arr_fl = np.append(zg_av_arr_fl, zg_av)
    
    np.save(savedir_photoz + 'firstlight_id_arr.npy', id_arr_fl)
    np.save(savedir_photoz + 'firstlight_field_arr.npy', field_arr_fl)
    np.save(savedir_photoz + 'firstlight_zs_arr.npy', zs_arr_fl)
    
    np.save(savedir_photoz + 'firstlight_zp_minchi2_arr.npy', zp_minchi2_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_arr.npy', zp_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_zerr_low_arr.npy', zp_zerr_low_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_zerr_up_arr.npy', zp_zerr_up_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_min_chi2_arr.npy', zp_min_chi2_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_bestalpha_arr.npy', zp_bestalpha_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_model_idx_arr.npy', zp_model_idx_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_age_arr.npy', zp_age_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_tau_arr.npy', zp_tau_arr_fl)
    np.save(savedir_photoz + 'firstlight_zp_av_arr.npy', zp_av_arr_fl)
    
    np.save(savedir_spz + 'firstlight_zspz_minchi2_arr.npy', zspz_minchi2_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_arr.npy', zspz_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_zerr_low_arr.npy', zspz_zerr_low_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_zerr_up_arr.npy', zspz_zerr_up_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_min_chi2_arr.npy', zspz_min_chi2_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_bestalpha_arr.npy', zspz_bestalpha_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_model_idx_arr.npy', zspz_model_idx_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_age_arr.npy', zspz_age_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_tau_arr.npy', zspz_tau_arr_fl)
    np.save(savedir_spz + 'firstlight_zspz_av_arr.npy', zspz_av_arr_fl)
    
    np.save(savedir_grismz + 'firstlight_zg_minchi2_arr.npy', zg_minchi2_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_arr.npy', zg_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_zerr_low_arr.npy', zg_zerr_low_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_zerr_up_arr.npy', zg_zerr_up_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_min_chi2_arr.npy', zg_min_chi2_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_bestalpha_arr.npy', zg_bestalpha_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_model_idx_arr.npy', zg_model_idx_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_age_arr.npy', zg_age_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_tau_arr.npy', zg_tau_arr_fl)
    np.save(savedir_grismz + 'firstlight_zg_av_arr.npy', zg_av_arr_fl)
...
def main():
    ...
    for j in range(44):  # doing the first 176 galaxies on firstlight --> 176 / 4 = 44
