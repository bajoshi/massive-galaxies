...
def get_err_and_d4000_arr(id_arr, field_arr, zgrism_arr, \
    ...
    for cat in allcats:
        ...
        for i in range(len(pears_unique_ids)):
            ...
            elif ~np.isfinite(current_zgrism) or (current_zgrism == -99.0) or (not current_zgrism):
                print "No valid zgrism:", current_zgrism, "Using zphot..."
                redshift = current_zphot
            
            if redshift < 0.6 or redshift > 1.235:
                print "Redshift out of range. Moving to next galaxy."
                continue

            print "At ID:", current_id, "in", current_field, "at redshift:", redshift
